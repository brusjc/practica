    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h2>{{ trans('esexamenes.exaB1_titulo200') }} {{$datos['mes']}} {{ trans('esexamenes.exaB1_titulo101') }} {{$datos['ano']}}</h2>
                @if(Auth::guest())
                    {{ trans('message.solo usuarios registrados') }}
                @endif
                <div class="pull-right">
                    @if(!Auth::guest())
                        <a href="/es/examennuevo/{{$codigo}}">
                    ...
                    @if(!Auth::guest())
                        @if($examenes['original']['status']['error']==0)
                            @foreach($examenes['original']['data'] as $examen)
                                <tr>
                                    <td><a href="/es/exameninicio/{{$examen['id']}}">{{$examen['prueba']['nombre']}}</a></td>
                                    <td><a href="/es/exameninicio/{{$examen['id']}}">{{date('d-m-Y', strtotime($examen['created_at']))}}</a></td>
                                    <td>

...
<div class="comment-text">
	<p>{{ trans('examenes.B1_texto4') }}</p>
	<p>{{ trans('examenes.B1_texto5') }} <a href="https://cvc.cervantes.es/ensenanza/biblioteca_ele/marco/" target="_blank">{{ trans('examenes.B1_texto6') }}</a></p>
</div>
								...
								@foreach($pruebas['original']['data'] as $prueba)
									...
									@foreach($prueba['examen'] as $examen)
										<tr>
											...
											<td>
												<span class="badge bg-primary text-center">{{$examen['porcentaje']}}%</span>
											</td>
										</tr>
									@endforeach
								@endforeach

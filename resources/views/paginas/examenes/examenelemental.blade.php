...
@section('contentheader_title')
    {{ trans('examenes.elemental_title') }}
@endsection

@section('contentheader_h1')
    {{ trans('examenes.elemental_titulo1') }}
@endsection
...
@section('descripcion')
    {{ trans('examenes.elemental_descripcion') }}
@endsection

@section('keywords')
    {{ trans('examenes.elemental_keywords') }}
@endsection

@section('main_content')

<div class="comment-text">
	<p>{{ html_entity_decode(trans('examenes.elemental_texto1') )}}</p>
	<p>{{ html_entity_decode(trans('examenes.elemental_texto2') )}}</p>
	<p>{{ html_entity_decode(trans('examenes.elemental_texto3') )}}</p>
	<p>{{ html_entity_decode(trans('examenes.elemental_texto4') )}}</p>
</div>
		...
		@if($pruebas['original']['status']['error']==0)
			@foreach($pruebas['original']['data'] as $key=>$nivel)
				<div class="col-lg-4 col-md-4">
					<a class="mb-2 btn btn-block btn-info" href="examen_elemental/{{$nivel['codigo']}}">{{$nivel['nombre']}}</a>
				</div>
			@endforeach
		@endif
	@endif
